...
add_action('plugins_loaded', 'backup_guard_plugins_loaded');
function backup_guard_plugins_loaded() {
	if (is_super_admin()) {
...
function backup_guard_manual_backup()
{
	...
	$filesToExclude = array_diff($backupFiles, @$_POST['directory']);

	if (is_array(@$_POST['directory']) && in_array('wp-content', $_POST['directory'])) {
		$_POST['directory'] = array('wp-content');
	}
	else {
		$filesToExclude = array_diff($filesToExclude, array('wp-content'));
		SGConfig::set('SG_BACKUP_FILE_PATHS', $_POST['directory'], false);
	}
	...
	SGConfig::set('SG_BACKUP_FILE_PATHS_EXCLUDE', SG_BACKUP_FILE_PATHS_EXCLUDE.$filesToExclude, true);
...
{
	if(@$_POST['backupType'] == SG_BACKUP_TYPE_FULL){
		SGConfig::set('SG_BACKUP_FILE_PATHS_EXCLUDE', SG_BACKUP_FILE_PATHS_EXCLUDE);
		SGConfig::set('SG_BACKUP_FILE_PATHS', 'wp-content');
	}
	elseif (@$_POST['backupType'] == SG_BACKUP_TYPE_CUSTOM){
		$backupFiles = explode(',', SG_BACKUP_FILE_PATHS);
		$filesToExclude = array_diff($backupFiles, @$_POST['directory']);
		...
		SGConfig::set('SG_BACKUP_FILE_PATHS_EXCLUDE', SG_BACKUP_FILE_PATHS_EXCLUDE.$filesToExclude);
	}
